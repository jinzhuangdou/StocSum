...
#         freq_strata<-freq_strata[,include]
#         # if(any(duplicated(null.obj$id_include))) geno <- crossprod(J, geno)
#         # if(!is.null(strata)) { # E is not continuous
#         #   freq.tmp <- sapply(strata.list, function(x) colMeans(geno[x, , drop = FALSE], na.rm = TRUE)/2) # freq.tmp is a matrix, each column is a strata, and each row is a varirant 
#         #   if (length(dim(freq.tmp)) == 2) freq_strata <- apply(freq.tmp, 1, range) else freq_strata <- as.matrix(range(freq.tmp)) # freq_strata is the range of allele freq across strata.list
...
#' @export
GE.svt.pval <- function(meta.files.prefix, out.file, n.files = rep(1, length(meta.files.prefix)), n.pheno = 1, interaction, interaction.covariates = NULL, MAF.range = c(1e-7, 0.5), miss.cutoff = 1, auto.flip = FALSE, nperbatch = 10000)
{
    if(.Platform$endian!="little") stop("Error: platform must be little endian.")
    ...
    for(i in 1:nbatch.flush) {
        ...
        # VK <- as.matrix(VK) * (matrix(1, ei, ei) %x% diag(ng))   #KPK=VK
        # VK_i <- try(solve(VK), silent = TRUE)
        # if(class(VK_i)[1] == "try-error") VK_i <- try(MASS::ginv(VK), silent = TRUE)
        # # if (class(VK_i)[1] == "try-error") {
        ...
        # #   return(fix_out[[3]])
        # # }
        # SK <- (rep(1, ei) %x% diag(ng)) * as.vector(SK)
        # BETA.K <- crossprod(VK_i, SK)

        # # ng1   <- ng+1
        # # ei1<-ei+1
        # # ngei1 <- ng*ei1
        # V.K_i <- try(solve(VK_i), silent = TRUE)
        # if(class(V.K_i)[1] == "try-error") V.K_i <- try(MASS::ginv(VK_i), silent = TRUE)
        ...
        # #     return(fix_out[[3]])
        # # }
        # STAT.K   <- diag(crossprod(BETA.K, crossprod(V.K_i, BETA.K)))

        # V_GK_i <- try(solve(V_GK), silent = TRUE)
        # if(class(V_GK_i)[1] == "try-error") V_GK_i <- try(MASS::ginv(V_GK), silent = TRUE)
...
GE.pval <- function(meta.files.prefix, n.files = rep(1, length(meta.files.prefix)), n.pheno = 1, cohort.group.idx = NULL, group.file, group.file.sep = "\t", interatcion, interaction.covariates = NULL, MAF.range = c(1e-7, 0.5), MAF.weights.beta = c(1, 25), miss.cutoff = 1, method = "davies", tests = "JF", rho = c(0, 0.1^2, 0.2^2, 0.3^2, 0.4^2, 0.5^2, 0.5, 1), use.minor.allele = FALSE, auto.flip = FALSE)
{
  if(.Platform$endian!="little") stop("Error: platform must be little endian.")
  n.cohort <- length(meta.files.prefix)
  if(length(n.files) != n.cohort) stop("Error: numbers of cohorts specified in meta.files.prefix and n.files do not match.")
