...
GE.stat <- function(null.obj, interaction, interaction.covariates = NULL, geno.file, meta.file.prefix, MAF.range = c(1e-7, 0.5), miss.cutoff = 1, missing.method = "impute2mean", nperbatch = 10000, ncores = 1)
{
  ...
  # saveRDS(residuals2, file="/HGCNT95FS/SummaryStat/work/SOL_GE/test/residuals2.RDS")
  ...
  if(ncores > 1) {
    ...
    for (ii in seq(1,ei)){
      for(i in 1:nbatch.flush) {
        ...
        # K <- do.call(cbind, sapply((1+qi):ncol(E), function(xx) geno*E[,xx], simplify = FALSE), envir = environment())
        K <- do.call(cbind, sapply(qi+ii, function(xx) geno*E[,xx], simplify = FALSE), envir = environment())
        # SK <- matrix(crosssprod(K,residuals), ncol=ei)
        # write.table(cbind(out[,c("SNP", "chr", "pos", "ref", "alt", "N", "missrate", "altfreq", "SCORE")],SK), meta.file.sample, quote=FALSE, row.names=FALSE, col.names=FALSE, append=TRUE, na=".")
        # writeBin(as.numeric(crossprod(residuals2, geno)), meta.file.resample.handle, size = 4)
        # for (ii in seq(1,ei)){
        writeBin(as.numeric(crossprod(residuals2, K[,((ii-1)*n.p+1):(ii*n.p)])), meta.file.resample.handle, size = 4)
...
#' out.file<-paste0(meta.files.prefix,".out")
#' GE.svt.pval(meta.files.prefix, out.file, n.files=ncores, n.pheno = 1, interaction=interaction, interaction.covariates=interaction.covariates, MAF.range=c(1e-7,0.5), miss.cutoff = 1, auto.flip=F)
#' }
...
GE.svt.pval <- function(meta.files.prefix, out.file, n.files = rep(1, length(meta.files.prefix)), n.pheno = 1, interaction, interaction.covariates = NULL, MAF.range = c(1e-7, 0.5), miss.cutoff = 1, auto.flip = FALSE, nperbatch = 100)
{
    ...
    nbatch.flush <- (p-1) %/% nperbatch + 1
    all.out <- NULL
    # write.table(t(c("SNP", "chr", "pos", "ref", "alt", "N", "missrate", "altfreq", "BETA.MAIN", "SE.MAIN", "PVAL.MAIN", paste("BETA.GxE",1:ei,sep=""), paste("SE.GE",1:ei,sep=""), paste("PVAL.GE",1:ei,sep=""), "PVAL.JOINT")), out.file, quote = FALSE, row.names = FALSE, col.names = FALSE)     
    ...
    for(i in 1:nbatch.flush) {
        ...
        # ng <- ncol(VG)
        # V_GK <- rbind(cbind(VG,t(VKG)),cbind(VKG,VK))
        # V_GK <- as.matrix(V_GK) * (matrix(1, 1+ei, 1+ei) %x% diag(ng))
        # U_GK <- c(UG, SK)

        # VG <- as.matrix(VG) * (matrix(1, 1, 1) %x% diag(ng))
        ...
        # PVAL.MAIN <- ifelse(VG_idiag>0, pchisq(out$STAT.MAIN, df=1, lower.tail=FALSE), NA)
        # out$PVAL.MAIN<-PVAL.MAIN
        out$SCORE.MAIN <- UG
        ...
        # Vkg <- Vgk
        # Vkk <-rowSums(VK_u^2)
        # denomi <- Vgg*Vkk - Vgk*Vkg
        # sco<-(Vgg*SK-Vkg*UG)/denomi
        # var<-Vgg/denomi
        # STAT.K <- sco^2/var
        # beta1<-(Vkk*UG-Vgk*SK)/denomi
        # beta2 <- (Vgg*SK-Vkg*UG)/denomi
        # STAT.JOINT<-Vgg*beta1^2+2*Vgk*beta1*beta2+Vkk*beta2^2
        ...
        # write.table(cbind(out[,c("SNP", "chr", "pos", "ref", "alt", "N", "missrate", "altfreq", "BETA.MAIN","SE.MAIN","PVAL.MAIN")], diag(BETA.GK[ng1:ngei1,]), diag(IV.K_i), PVAL.K, PVAL.JOINT), out.file, quote=FALSE, row.names=FALSE, col.names=FALSE, append=TRUE, na="NA")
...
GE.pval <- function(meta.files.prefix, n.files = rep(1, length(meta.files.prefix)), n.pheno = 1, cohort.group.idx = NULL, group.file, group.file.sep = "\t", interatcion, MAF.range = c(1e-7, 0.5), MAF.weights.beta = c(1, 25), miss.cutoff = 1, method = "davies", tests = "JF", rho = c(0, 0.1^2, 0.2^2, 0.3^2, 0.4^2, 0.5^2, 0.5, 1), use.minor.allele = FALSE, auto.flip = FALSE)
{
  ...
  JD <- "JD" %in% tests
  if(!class(interaction) %in% c("integer", "numeric", "character")) stop("Error: \"interaction\" should be an integer, numeric, or character vector.")
  qi <- 0  #length(interaction.covariates) # number of covariates with interaction effects but we don't test
  ei <- length(interaction) # number of covariates with interaction effects that we want to test
  interaction <- as.character(interaction)
  n.E = qi + ei
  ...
  for(i in 1:n.groups) {
    ...
    U <- U*tmp.weight
    for (k in seq(1,(1+n.E))) {   #    # for (k in seq((1+qi),(1+n.E))){
      V[[k]] <- V[[k]]*tmp.weight
    }
    UG<-as.vector(U[,1:(1+qi)])     #U of geno and interaction.covariates
    VG_u<-do.call(rbind, lapply(V[1:(1+qi)], unlist))  #rbind(V[[1:(1+qi)]]); 
    VG<-tcrossprod(VG_u)       #covariance matrix V of geno and interaction.covariates
    if(IV | IF | JV | JF | JD) {
      SK <- as.vector(U[,(1+qi+1):(1+n.E)])
      VG_u<-do.call(rbind, lapply(V[1:n.pheno], unlist)) 
      VK_u<-do.call(rbind, lapply(V[(1+qi+1):(1+n.E)], unlist)) # rbind(V[[(1+qi+1):(1+n.E)]]) #
      ...
      VG_i <- try(solve(VG), silent = TRUE)
      if(class(VG_i)[1] == "try-error") VG_i <- MASS::ginv(VG)
      IV.U<-SK-tcrossprod(tcrossprod(VKG,VG_i),t(UG))
      # IV.V<-VK-tcrossprod(tcrossprod(VKG,VG_i),VKG)
      IV.V_u<-VK_u-tcrossprod(tcrossprod(VKG,VG_i),t(VG_u))
    }

    if(MV | JV) MV.pval[i] <- tryCatch(.quad_Vpval(U = UG, V = VG_u, method = method), error = function(e) { NA })
    if(IV | JV) IV.pval[i] <- tryCatch(.quad_Vpval(U = IV.U, V = IV.V_u, method = method), error = function(e) { NA })
    if(JV && is.null(interaction.covariates)) JV.pval[i] <- tryCatch(fisher_pval(c(MV.pval[i], IV.pval[i])), error = function(e) { MV.pval[i] })
    
    if(MF | JF | JD) {
      MF.BU <- sum(UG)
      MF.BV <- sum(colSums(VG_u)^2) ##sum(VG)
      MF.Bp <- pchisq(MF.BU^2/MF.BV,df=1,lower.tail=FALSE)
      #V.rowSums <- if(length(VG)>1) rowSums(VG) else VG[1]
      Vsum <- colSums(VG_u)
      V.rowSums <- crossprod(t(VG_u),Vsum)
      MF.U <- UG - V.rowSums * MF.BU / MF.BV
      #MF.V <- VG - tcrossprod(V.rowSums) / MF.BV
      MF.V_u <- VG_u - crossprod(crossprod(Vsum,t(VG_u)),t(Vsum)) / MF.BV
      if(MF.BV == 0 | mean(abs(MF.V_u)) < sqrt(.Machine$double.eps)) MF.p <- NA
      else MF.p <- tryCatch(.quad_Vpval(U = MF.U, V = MF.V_u, method = method), error = function(e) { NA })
    ...
    if(IF | JF | JD) {
      IF.BU <- sum(IV.U)
      # IF.BV <- sum(IV.V)
      IF.BV <- sum(colSums(IV.V_u)^2)
      IF.Bp <- pchisq(IF.BU^2/IF.BV,df=1,lower.tail=FALSE)
      # IV.V.rowSums <- if(length(IV.V)>1) rowSums(IV.V) else IV.V[1]
      Vsum <- colSums(IV.V_u)
      IV.V.rowSums <- crossprod(t(IV.V_u),Vsum)
      IF.U <- IV.U - IV.V.rowSums * IF.BU / IF.BV
      # IF.V <- IV.V - tcrossprod(IV.V.rowSums) / IF.BV
      IF.V_u <- IV.V_u - crossprod(crossprod(Vsum,t(IV.V_u)),t(Vsum)) / IF.BV
      if(IF.BV == 0 | mean(abs(IF.V_u)) < sqrt(.Machine$double.eps)) IF.p <- NA
      else IF.p <- tryCatch(.quad_Vpval(U = IF.U, V = IF.V_u, method = method), error = function(e) { NA })
